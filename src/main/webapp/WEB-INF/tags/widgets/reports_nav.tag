...
<div class="nav">
    ...
    <c:if test="${not (subNavItem eq 'graph')}"><a href="/sources/${name}/reports/graph">graph</a></c:if>

    <c:if test="${subNavItem eq 'stats'}">- stats</c:if>
        <c:if test="${not (subNavItem eq 'stats')}">- <a href="/sources/${name}/reports/stats">stats</a></c:if>

        <c:if test="${subNavItem eq 'table'}">- table</c:if>
    <c:if test="${not (subNavItem eq 'table')}">- <a href="/sources/${name}/reports/table">table</a></c:if>
</div>